    ...
    try {
      ...
      try {
        ...
        if (chat) {
          ...
          if (!('antiUrl' in chat)) chat.antiUrl = false
      ...
      for (let name in global.plugins) {
        ...
        if (!plugin) continue
        if (plugin.disabled) continue
        if (!opts['restrict']) if (plugin.tags && plugin.tags.includes('admins')) continue
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    admin: 'This command is for *Admin* of the group only!',
